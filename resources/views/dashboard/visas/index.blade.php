                ...
                <div class="iq-card">
                   ...
                   <div class="iq-card-body">
                      <div class="table-responsive">
                        <a class="btn btn-primary" style="margin-bottom: 15px" href="{{ route('dashboard.visas.create') }}">Add Visa</a>
                         ...
                         <table id="user-list-table" class="table table-striped table-bordered mt-4" role="grid" aria-describedby="user-list-page-info">
                           ...
                           <tbody>
                            @foreach ($visas as $visa)
                                <tr>
                                    <td class="text-center"><img class="rounded img-fluid avatar-40" src="{{ asset('storage/'.$visa->image) }}" alt="profile"></td>
                                    <td>{{ $visa->title_en }}</td>
                                    <td>{{ $visa->duration }}</td>
                                    <td>{{ $visa->country->title_en }}</td>
                                    <td>{{ $visa->visa_type }}</td>
                                    <td>{{ $visa->price }}</td>
                                    <td>{{ $visa->quantity }}</td>
                                    <td>
                                        <div class="flex align-items-center list-user-action">
                                            <a class="iq-bg-primary" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit" href="{{ route('dashboard.visas.edit',$visa->id) }}"><i class="ri-pencil-line"></i></a>
                                            <form method="POST" action="{{ route('dashboard.visas.destroy',$visa->id) }}">
                                                @csrf
                                                @method('delete')
